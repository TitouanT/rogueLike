			...
			case '\n':
				if(map[player.line][player.column].nbObject > 0){
					switch (map[player.line][player.column].obj[0]) {
						case STAIRS_UP: writeLvl(map,(player.lvl)); (player.lvl)++; readLvl(map,(player.lvl)); move2spawn(map, &player);  break;
						case STAIRS_DOWN: writeLvl(map,(player.lvl)); (player.lvl)--; readLvl(map,(player.lvl)); move2Stairs_UP(map, &player); break;

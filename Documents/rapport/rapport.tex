...
Dans cette partie nous présentons les structures utilisées dans le code, l'organisation interne du code ainsi que quelques détails sur les fonctionnalités importantes du jeu.
	...
	Voici un descriptif des principales fonctionnalités du programme, et de leur mise en place dans notre projet.
		...
		\end{enumerate}
			...
			Pour placer les pièces, nous les plaçons les unes après les autres. Pour chaque pièce, nous commençons par lui donner une position (la position de son angle supérieur gauche). Ensuite nous lui choisissons une dimension (hauteur, largeur) afin qu'elle entre la plus petite et la plus grande pièce possible.

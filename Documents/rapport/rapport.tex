...
\end{table}
	...
	\section{Doxygen, CUnit, GDB}\label{gdb}
		...
		Enfin, lorsque nous avons certains bogues que nous n'arrivons pas à résoudre, nous utilisons le logiciel de débogage \emph{GDB} (\emph{GNU DeBugger}). Nous n'avons pas réussi à le faire fonctionner dès le début, car nous utilisons la libraire d'affichage \emph{ncurses}\footnote{Voir la section \hyperref[ncurses]{interactions et déplacements, page \getpagerefnumber{ncurses}}.}, qui utilise déjà le terminal pour afficher notre jeu. En la combinant avec \emph{GDB}, le terminal n'était plus utilisable.\\
...
\chapter*{Annexe}
	...
	\section*{Cas d'utilisation du débogueur \emph{GDB}}
		...
		Parfois, nous n'arrivons pas à trouver la cause de certains bugs, qui étaient finalement très simples. Pour cela, nous avons utilisé l'outil \emph{GDB}, comme mentionné dans \hyperref[gdb]{la partie sur les outils, page \getpagerefnumber{gdb}}. Afin de l'utiliser avec \emph{ncurses}, voici la marche à suivre :
